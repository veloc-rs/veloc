...
use veloc_semantics::{BvOp, Error, Expr, Function, IntPredicate, Sort, Trap, equivalence_query};

fn main() -> Result<(), Error> {
    let specs = offline::SPECS;
    let program = |op: Opcode| {
        specs
            .iter()
            .find(|s| s.opcode == op)
    ...
    let (source, target) = match mode {
        "division" => {
            let source = program(Opcode::IDivS).instantiate(&[sort, sort], &[sort], &[])?;
            ...
            let overflow =
                quotient.compare(&result.sign_extend(bits * 2)?, IntPredicate::new(false, 5))?;
        ...
        "shift" => {
            let source = program(Opcode::IShrS).instantiate(&[sort, sort], &[sort], &[])?;
        ...
        "overflow" => {
            let source = program(Opcode::IAddWithOverflow).instantiate(
                &[sort, sort],
                &[sort, Sort::Bool],
                &[],
            )?;
        ...
        "comparison" => {
            let source = program(Opcode::Icmp).instantiate(
                &[sort, sort],
                &[Sort::Bool],
                &[offline::predicate(IntCC::LtS)],
            )?;
        ...
        "zext" | "sext" | "trunc" => {
            ...
            let source = program(op).instantiate(&[sort], &[Sort::bv(width)?], &[])?;
