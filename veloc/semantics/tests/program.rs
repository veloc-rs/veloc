...
fn composed_negation_agrees_with_the_primitive_at_every_width() {
    ...
    for bits in 1..=128 {
        ...
        let function = COMPOSED_NEG.instantiate(width).unwrap();
...
fn primitive_recognition_uses_logical_input_order() {
    const SUB: Program = Program {
        inputs: 2,
        steps: &[
            Step::Const(BvConst::One), // Unused, but valid.
            Step::Input(1),
            Step::Input(0),
            Step::Apply {
                op: BvOp::Sub,
            ...
            Step::Const(BvConst::Zero), // The output need not be the last step.
        ],
        output: 3,
    };
    ...
    assert_eq!(Program { output: 0, ..SUB }.primitive(), None);
...
fn constant_and_identity_programs_preserve_their_signatures() {
    const CONSTANT: Program = Program {
        inputs: 0,
        steps: &[Step::Const(BvConst::AllOnes)],
        output: 0,
    };
    ...
    assert_eq!(
        CONSTANT.instantiate(width).unwrap().eval(&[]),
        Ok(Value::Bv(255))
    );
    assert_eq!(IDENTITY.eval(width, &[257, 0]), Ok(1));
    assert_eq!(
        IDENTITY.instantiate(width).unwrap().inputs(),
        &[Sort::Bv(width); 2]
    );
...
fn rejects_invalid_step_references_and_lengths_without_panicking() {
    ...
    for program in INVALID {
        ...
        assert!(program.instantiate(width).is_err(), "{program:?}");
...
fn step_count_must_fit_the_reference_representation() {
    ...
    static TOO_LONG: [Step; MAX + 1] = [Step::Const(BvConst::Zero); MAX + 1];
    static FULL: [Step; MAX] = [Step::Const(BvConst::One); MAX];
    let program = Program {
        inputs: 0,
        steps: &TOO_LONG,
        output: 0,
    ...
    assert!(program.instantiate(Width::new(8).unwrap()).is_err());
...
fn composed_programs_share_the_existing_smt_encoder() {
    ...
    let query = equivalence_query(
        &NEG.instantiate(width).unwrap(),
        &COMPOSED_NEG.instantiate(width).unwrap(),
    )
    .unwrap();
