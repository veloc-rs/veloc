...
use veloc_types::{
    CallConv, CallableKind, ScalarType, SigId, Signature, SignatureError, Type, TypeContext,
    TypeInfo,
};
...
fn interning_preserves_payload_and_parameter_result_boundary() {
    let mut pool = TypeContext::default();
    let signature = Signature::new([Type::I32], [Type::I64], CC);
    let payload = signature.types().as_ptr();
    let id = pool.insert_signature(signature);
    assert_eq!(pool.signatures()[id].types().as_ptr(), payload);
    assert_eq!(pool.intern_signature(&[Type::I32], &[Type::I64], CC), id);
    assert_ne!(pool.intern_signature(&[Type::I32, Type::I64], &[], CC), id);
    // Rehashing must use the same identity as borrowed lookup.
    for count in 0..1000 {
        let params = vec![Type::I32; count];
        let id = pool.intern_signature(&params, &[], CC);
        assert_eq!(pool.insert_signature(Signature::new(&params, [], CC)), id);
    }
    assert_eq!(pool.intern_signature(&[Type::I32], &[Type::I64], CC), id);
    assert_eq!(pool.signatures()[id].types().as_ptr(), payload);
    let mut clone = pool.clone();
    assert_eq!(clone.intern_signature(&[Type::I32], &[Type::I64], CC), id);
    assert_eq!(pool.signatures()[id].params(), &[Type::I32]);
    assert_eq!(pool.signatures()[id].returns(), &[Type::I64]);
}

#[test]
fn import_remaps_nested_signatures_and_rejects_invalid_graphs_atomically() {
    let mut source = TypeContext::default();
    // Forward reference: import must process the leaf before the outer signature.
    let outer = source.intern_signature(&[Type::callable(SigId(1), CallableKind::Owned)], &[], CC);
    let leaf = source.intern_signature(&[Type::I32], &[Type::I64], CC);
    assert_eq!(leaf, SigId(1));
    let mut target = TypeContext::default();
    let canonical_leaf = target.intern_signature(&[Type::I32], &[Type::I64], CC);
    let canonical_outer = target.intern_signature(
    ...
    // Equal raw integers across contexts need not denote equal signatures.
    assert_eq!(outer, canonical_leaf);
    assert_ne!(ids[outer.0 as usize], canonical_leaf);
    let shared = target.intern_signature(
        &[Type::callable(canonical_leaf, CallableKind::Shared)],
        &[],
        CC,
    );
    assert_ne!(shared, canonical_outer);
    let different = target.intern_signature(&[Type::I64], &[Type::I32], CC);
    ...
    for (reference, expected) in [
        ...
        let mut bad = TypeContext::default();
        bad.intern_signature(&[Type::callable(reference, CallableKind::Shared)], &[], CC);
        let before = target.signatures().len();
        assert_eq!(target.import(&bad), Err(expected));
        assert_eq!(target.signatures().len(), before);
    }
}
