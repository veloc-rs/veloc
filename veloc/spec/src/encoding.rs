...
impl TypeEncoding {
    ...
    pub fn generate(&self) -> String {
        let mut out = String::from(
            "// @generated by veloc-opgen; edit defs/types.ops.\n/// Tagged MIR type. These fields describe its compact scalar/vector payload;\n/// structural types use a disjoint tag and module-interned signature.\n",
        );
        ...
        for name in self.layout.fields.keys() {
            ...
            let value = match result {
                "bool" => format!("self.0 & ({field}_MASK as u64) != 0"),
                "u8" => format!("((self.0 & ({field}_MASK as u64)) >> {field}_SHIFT) as u8"),
                _ => format!("((self.0 & ({field}_MASK as u64)) >> {field}_SHIFT) as u16"),
            };
            writeln!(
                out,
        ...
        out.push_str("/// Compact scalar/vector encoding. Structural types have no compact encoding.\npub const fn to_raw(self) -> u16 { assert!(self.is_compact(), \"structural type has no compact encoding\"); self.0 as u16 }\n");
        out.push_str("/// Return the element type. Structural types have no element.\npub const fn element_type(self) -> Self { assert!(self.is_compact(), \"structural type has no element\"); Self(self.0 & (SCALAR_MASK as u64)) }\n}\n");
        out
    }
}
