...
pub(crate) fn generate(
    ...
    let params = contexts
        ...
        .collect::<String>();
    let mut out = format!(
        "// @generated by veloc-spec. Edit the .spec definitions instead.\nimpl FunctionRef<'_> {{\n    fn validate_constraints(&self, _dfg: &crate::dfg::DataFlowGraph, _module: &crate::Module, _inst: Inst, data: &InstView<'_>, _operands: &[Type], _results: &[Type]{params}) -> Result<()> {{\n",
    );
            ...
            |source, value| {
                let id = match source {
                    SignatureSource::Function(_) => {
                        format!("_module.decls.get({value}).ok_or_else(|| {error})?.signature")
                    }
