...
pub(crate) fn compile(records: &[Record], source: &str) -> Result<Encodings, Error> {
    ...
    for record in records.iter().filter(|r| r.kind == "encoding") {
        ...
        fields.finish()?;
        defs.insert(record.name.clone(), layout);
    }
    Ok(defs)
}

pub(crate) fn generate(defs: &Encodings) -> String {
    let mut out = String::from("// @generated by veloc-opgen; edit the .ops definitions.\n");
    for (name, layout) in defs {
...
/// Layout checks for explicit packed bit fields.
#[derive(Debug)]
