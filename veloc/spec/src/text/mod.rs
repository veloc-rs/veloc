...
pub(crate) fn generate(defs: &Definitions, source: &str) -> Result<(String, String), Error> {
    let mut parser = String::from(
        "// @generated by veloc-opgen; edit the .ops definitions.\nimpl OperandParser<'_> {\nfn parse(&mut self, opcode: crate::Opcode, ty: Option<crate::Type>, flags: crate::MemFlags, text: &str) -> core::result::Result<crate::InstDraft, ParseError> {\nmatch opcode {\n",
    );
...
fn alternate(
    ...
    for field in &alt.fields {
        ...
        let (kind, binding) = match ty.as_str() {
            ...
            _ => match defs.storage.records.iter().find(|r| r.name == *ty) {
                Some(record) => (
