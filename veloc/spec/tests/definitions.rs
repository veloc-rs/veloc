...
"#;
    for (source, message) in [
        ...
        ("storage Operands {} struct Unary { dst: Def, src: Use } op G_COPY<T: Integer>(src: T) -> T { meta: OpInfo { memory: Known([]) }, storage: Unary, verify {require(true, \"ok\");
} }".into(), "operand storage does not yet support"),
    ] {
        let source = common::source(&source);
