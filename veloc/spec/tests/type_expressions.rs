...
fn precedence_parentheses_and_equivalent_sets_share_runtime_checks() {
    for (expression, equivalent) in [
        ...
        let actual = compile(&source(expression)).unwrap();
        let expected = compile(&source(equivalent)).unwrap();
...
fn named_aliases_and_inline_expressions_intern_to_the_same_set() {
    ...
    let named = compile(&source).unwrap();
    let inline = compile(&source.replace("T: Wide", "T: I64 | I32")).unwrap();
    ...
    let aliases =
        compile("class A { members: [I32 | I64] } class B { members: [I64 | I32] }").unwrap();
    let id = |name| {
        aliases
            .opcodes
            .lines()
